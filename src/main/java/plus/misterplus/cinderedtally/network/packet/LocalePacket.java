...
import plus.misterplus.cinderedtally.helper.CommandHelper;
...
public class LocalePacket {
    ...
    public LocalePacket() {
        this.msgKick = I18n.get("msg.cinderedtally.kick");
        this.msgBanned = I18n.get("msg.cinderedtally.banned");
    }
    ...
    public static void handle(LocalePacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (LifespanHelper.getLifespan(player) == 0) {
                CinderedTally.LOGGER.debug(String.format("Player %s out of time, kicking...", name));
